 ...
 *
 * 问题描述
 * 有N个点，保证每个点之间都有路径可达，计算从起点出发然后回到起点的最短路径，每个点可以经过多次。
 * 问题分析
 * 和旅行家问题不同之处在于这里可以重复经过一个点，所以在已经过集合S中添加一个点时，不是考虑v是否能直接达到u，而是v到u的最短距离。
 * 注意点：S从空集开始递增才能保证dp的无后效性。也可以使用长度为K的子集，递增K保证无后效性。
 */
